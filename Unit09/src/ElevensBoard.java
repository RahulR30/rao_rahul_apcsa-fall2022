...
public class ElevensBoard extends Board {
	...
	public boolean anotherPlayIsPossible() {
		...
		if(!containsPairSum11(index)){
			return containsJQK(index);
		}
		return true;
	}
	...
	private boolean containsPairSum11(List<Integer> selectedCards) {
		...
		for(int x = 0; x < selectedCards.size() - 1; x++){
			for(int y = x +1; x < selectedCards.size(); y++){
	...
	private boolean containsJQK(List<Integer> selectedCards) {
		...
		for (Integer selectedCard : selectedCards) {
			if (Objects.equals(cardAt(selectedCard).rank(), "jack")) {
				jack = true;
			} else if (Objects.equals(cardAt(selectedCard).rank(), "queen")) {
				queen = true;
